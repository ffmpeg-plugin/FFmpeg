...
if(ENABLE_LIBPLACEBO)
    ...
    # Check libplacebo API version compatibility
    # FFmpeg code is incompatible with libplacebo 7.x (API 339+)
    if(CONFIG_LIBPLACEBO AND PC_LIBPLACEBO_CFLAGS)
        # Extract include paths from CFLAGS
        set(_pl_inc_dirs "")
        foreach(_flag IN LISTS PC_LIBPLACEBO_CFLAGS)
            if(_flag MATCHES "^-I(.+)$")
                list(APPEND _pl_inc_dirs "${CMAKE_MATCH_1}")
            endif()
        endforeach()
        
        # Look for config.h to check API version
        set(_pl_api_ver 0)
        foreach(_inc_dir IN LISTS _pl_inc_dirs)
            if(EXISTS "${_inc_dir}/libplacebo/config.h")
                file(STRINGS "${_inc_dir}/libplacebo/config.h" _pl_config_lines REGEX "^#define PL_API_VER")
                foreach(_line IN LISTS _pl_config_lines)
                    if(_line MATCHES "PL_API_VER ([0-9]+)")
                        set(_pl_api_ver "${CMAKE_MATCH_1}")
                    endif()
                endforeach()
                break()
            endif()
        ...
        if(_pl_api_ver GREATER_EQUAL 339)
            message(STATUS "libplacebo API version ${_pl_api_ver} detected (7.x), disabling due to API incompatibility")
            set(CONFIG_LIBPLACEBO 0)
            set(libplacebo_found FALSE)
        endif()
    endif()
else()
    set(CONFIG_LIBPLACEBO 0)
...
if(ENABLE_LIBWEBP)
    ...
    # libwebp_anim_encoder needs libwebpmux (configure line 7433)
    if(CONFIG_LIBWEBP_ANIM_ENCODER)
        ffmpeg_check_pkg_config(libwebp_anim_encoder "libwebpmux >= 0.4.0" "webp/mux.h" "WebPAnimEncoderOptionsInit")
    endif()
else()
...
if(APPLE)
    ...
    if(ENABLE_SECURETRANSPORT)
        ...
        if(SECURITY_FRAMEWORK)
            set(CONFIG_SECURETRANSPORT 1 CACHE INTERNAL "SecureTransport enabled")
        else()
            set(CONFIG_SECURETRANSPORT 0 CACHE INTERNAL "SecureTransport not found")
        endif()
    else()
        set(CONFIG_SECURETRANSPORT 0 CACHE INTERNAL "SecureTransport disabled")
    endif()
else()
    set(CONFIG_VIDEOTOOLBOX 0)
    set(CONFIG_AUDIOTOOLBOX 0)
    ...
    set(CONFIG_APPKIT 0)
    set(CONFIG_AVFOUNDATION 0)
    set(CONFIG_SECURETRANSPORT 0 CACHE INTERNAL "SecureTransport not available (non-Apple)")
endif()
...
# SChannel (Windows TLS)
if(WIN32)
    option(ENABLE_SCHANNEL "Enable SChannel (Windows TLS)" ON)
    if(ENABLE_SCHANNEL)
        set(CONFIG_SCHANNEL 1 CACHE INTERNAL "SChannel enabled")
    else()
        set(CONFIG_SCHANNEL 0 CACHE INTERNAL "SChannel disabled")
    endif()
else()
    set(CONFIG_SCHANNEL 0 CACHE INTERNAL "SChannel not available (non-Windows)")
endif()
