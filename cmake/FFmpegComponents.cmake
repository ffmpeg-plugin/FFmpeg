...
# --------------------------------------------------------------------------
# codec_list.c - only include enabled codecs
# --------------------------------------------------------------------------
set(CODEC_ENTRIES "")
foreach(ENCODER IN LISTS ALL_ENCODERS)
    string(TOUPPER "${ENCODER}_ENCODER" _cfg)
    if(CONFIG_${_cfg})
        string(APPEND CODEC_ENTRIES "    &ff_${ENCODER}_encoder,\n")
    endif()
endforeach()
foreach(DECODER IN LISTS ALL_DECODERS)
    string(TOUPPER "${DECODER}_DECODER" _cfg)
    if(CONFIG_${_cfg})
        string(APPEND CODEC_ENTRIES "    &ff_${DECODER}_decoder,\n")
    endif()
endforeach()

set(CODEC_LIST_CONTENT "static const FFCodec * const codec_list[] = {\n")
string(APPEND CODEC_LIST_CONTENT "${CODEC_ENTRIES}    NULL };\n")
file(WRITE "${CMAKE_BINARY_DIR}/libavcodec/codec_list.c" "${CODEC_LIST_CONTENT}")
...
# --------------------------------------------------------------------------
# bsf_list.c - only include enabled BSFs
# --------------------------------------------------------------------------
set(BSF_ENTRIES "")
if(EXISTS "${CMAKE_SOURCE_DIR}/libavcodec/bitstream_filters.c")
    file(STRINGS "${CMAKE_SOURCE_DIR}/libavcodec/bitstream_filters.c" BSF_LINES2 REGEX "extern const FFBitStreamFilter ff_")
    foreach(LINE IN LISTS BSF_LINES2)
        if(LINE MATCHES "extern const FFBitStreamFilter (ff_([a-z0-9_]+)_bsf)")
            set(BSF_SYM "${CMAKE_MATCH_1}")
            set(BSF_NAME "${CMAKE_MATCH_2}")
            string(TOUPPER "${BSF_NAME}_BSF" _cfg)
            if(CONFIG_${_cfg})
                string(APPEND BSF_ENTRIES "    &${BSF_SYM},\n")
            endif()
        endif()
    endforeach()
endif()

set(BSF_LIST_CONTENT "/* Auto-generated by CMake - do not edit! */\n")
string(APPEND BSF_LIST_CONTENT "static const FFBitStreamFilter * const bitstream_filters[] = {\n")
string(APPEND BSF_LIST_CONTENT "${BSF_ENTRIES}")
string(APPEND BSF_LIST_CONTENT "    NULL\n};\n")
file(WRITE "${CMAKE_BINARY_DIR}/libavcodec/bsf_list.c" "${BSF_LIST_CONTENT}")
