...
#
# ffmpeg_require_pkg_config_cxx:
#   Check for C++ library using pkg-config (mimics require_pkg_config_cxx)
#
macro(ffmpeg_require_pkg_config_cxx name pkg_version headers classes)
    string(TOUPPER "${name}" _rpc_name_upper)
    string(TOLOWER "${name}" _rpc_name_lower)
    ...
    if(_rpc_skip_check)
        ...
        if(PC_${_rpc_name_upper}_FOUND)
            # Pkg-config found the package, now verify headers/classes
            set(CMAKE_REQUIRED_INCLUDES ${PC_${_rpc_name_upper}_INCLUDE_DIRS})
            set(CMAKE_REQUIRED_LIBRARIES ${PC_${_rpc_name_upper}_LIBRARIES})
            set(CMAKE_REQUIRED_FLAGS ${PC_${_rpc_name_upper}_CFLAGS_OTHER})
...
int main() {
")
            foreach(_cls ${classes})
                string(APPEND _rpc_test_code "    ${_cls}* p = nullptr;\n")
            endforeach()
            string(APPEND _rpc_test_code "    return 0;\n}\n")
            
            check_cxx_source_compiles("${_rpc_test_code}" HAVE_${_rpc_name_upper}_PC)
            
            if(HAVE_${_rpc_name_upper}_PC)
                set(CONFIG_${_rpc_name_upper} 1)
                set(${_rpc_name_lower}_found TRUE)
                set(${_rpc_name_lower}_cflags "${PC_${_rpc_name_upper}_CFLAGS}")
                set(${_rpc_name_lower}_extralibs "${PC_${_rpc_name_upper}_LDFLAGS}")
                set(${_rpc_name_upper}_INCLUDE_DIRS "${PC_${_rpc_name_upper}_INCLUDE_DIRS}")
                set(${_rpc_name_upper}_LIBRARIES "${PC_${_rpc_name_upper}_LIBRARIES}")
                message(STATUS "Checking for ${name} (C++): yes (pkg-config)")
            else()
                set(CONFIG_${_rpc_name_upper} 0)
                set(${_rpc_name_lower}_found FALSE)
...
#
# FFmpegDetectLibx264:
#   Detect x264 encoder library using CMake standard pkg_check_modules
#
macro(FFmpegDetectLibx264)
    if(ENABLE_LIBX264)
        find_package(PkgConfig QUIET)
        pkg_check_modules(LIBX264 QUIET x264>=0.155 IMPORTED_TARGET)
        
        if(LIBX264_FOUND)
            set(CONFIG_LIBX264 1)
            # Parse X264_BUILD from version (e.g., "0.164.x" → 164)
            if(LIBX264_VERSION)
                string(REGEX MATCH "^0\.([0-9]+)" _x264_ver "${LIBX264_VERSION}")
                if(_x264_ver AND CMAKE_MATCH_1 GREATER_EQUAL 163)
                    set(CONFIG_LIBX264_HDR10 1)
                else()
            ...
            message(STATUS "Found x264: ${LIBX264_VERSION} (HDR10=${CONFIG_LIBX264_HDR10})")
        else()
...
#   Based on configure's: require_pkg_config vaapi "libva >= 0.35.0" "va/va.h" vaInitialize
#
macro(FFmpegDetectVAAPI)
    if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
        if(NOT DEFINED ENABLE_VAAPI OR ENABLE_VAAPI)
            # Try pkg-config first (like configure does)
            ffmpeg_require_pkg_config(vaapi libva "va/va.h" "vaInitialize")
            
            # If pkg-config fails, fall back to header-only detection
            if(NOT CONFIG_VAAPI)
                check_include_file("va/va.h" HAVE_VA_VA_H)
                if(HAVE_VA_VA_H)
                    message(STATUS "Checking for vaapi: yes (header only)")
                    set(CONFIG_VAAPI 1)
            ...
            # Use CACHE FORCE to override FFmpegDefaults.cmake defaults
            set(CONFIG_VAAPI ${CONFIG_VAAPI} CACHE INTERNAL "FFmpeg VAAPI config" FORCE)
            set(VAAPI_FOUND ${VAAPI_FOUND} CACHE INTERNAL "VAAPI found" FORCE)
            set(VAAPI_LIBRARIES ${VAAPI_LIBRARIES} CACHE INTERNAL "VAAPI libraries" FORCE)
            set(VAAPI_INCLUDE_DIRS ${VAAPI_INCLUDE_DIRS} CACHE INTERNAL "VAAPI include dirs" FORCE)
            set(VAAPI_LIBRARY_DIRS ${VAAPI_LIBRARY_DIRS} CACHE INTERNAL "VAAPI library dirs" FORCE)
            
            if(CONFIG_VAAPI)
                # Check for specific VaAPI codecs (like configure's check_type)
                check_c_source_compiles("
                    #include <va/va.h>
                    #include <va/va_dec_h264.h>
                    int main(void) { return 0; }
                " HAVE_VAAPI_DEC_H264)
                if(HAVE_VAAPI_DEC_H264)
                    set(CONFIG_VAAPI_DEC_H264 1)
                else()
                    set(CONFIG_VAAPI_DEC_H264 0)
                endif()
                ...
                check_c_source_compiles("
                    ...
                    int main(void) { return 0; }
                " HAVE_VAAPI_DEC_HEVC)
                if(HAVE_VAAPI_DEC_HEVC)
                    set(CONFIG_VAAPI_DEC_HEVC 1)
                else()
                    set(CONFIG_VAAPI_DEC_HEVC 0)
                endif()
            endif()
        else()
            set(CONFIG_VAAPI 0)
...
#   Based on configure's: check_cpp_condition vdpau vdpau/vdpau.h "defined VDP_DECODER_PROFILE_MPEG4_PART2_ASP"
#
macro(FFmpegDetectVDPAU)
    if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
        if(NOT DEFINED ENABLE_VDPAU OR ENABLE_VDPAU)
            # Try pkg-config first (like configure does with check_pkg_config)
            ffmpeg_check_pkg_config(vdpau vdpau "vdpau/vdpau.h" "vdp_device_create_x11")
            
            # If pkg-config fails, fall back to header-only detection
            # configure uses: check_type "vdpau/vdpau.h" "VdpPictureInfoHEVC"
            if(NOT CONFIG_VDPAU)
                check_include_file("vdpau/vdpau.h" HAVE_VDPAU_VDPAU_H)
                if(HAVE_VDPAU_VDPAU_H)
                    # Verify it's a working VDPAU installation by checking for a type
                    check_c_source_compiles("
                        #include <vdpau/vdpau.h>
                        VdpDevice device;
                        int main(void) { return 0; }
                    " HAVE_VDPAU_DEVICE)
                    if(HAVE_VDPAU_DEVICE)
                        message(STATUS "Checking for vdpau: yes (header only)")
                        set(CONFIG_VDPAU 1)
            ...
            # Propagate variables from nested call to grandparent scope
            # Use CACHE FORCE to override FFmpegDefaults.cmake defaults
            set(CONFIG_VDPAU ${CONFIG_VDPAU} CACHE INTERNAL "FFmpeg VDPAU config" FORCE)
            set(VDPAU_FOUND ${VDPAU_FOUND} CACHE INTERNAL "VDPAU found" FORCE)
            set(VDPAU_LIBRARIES ${VDPAU_LIBRARIES} CACHE INTERNAL "VDPAU libraries" FORCE)
            set(VDPAU_INCLUDE_DIRS ${VDPAU_INCLUDE_DIRS} CACHE INTERNAL "VDPAU include dirs" FORCE)
        else()
            set(CONFIG_VDPAU 0)
        endif()
    else()
